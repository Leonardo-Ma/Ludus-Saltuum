...
static func build_save(data: CheckpointSaveData) -> void:
	...
	var should_save: bool = checkpoint != null and checkpoint.should_save_checkpoint
	data.has_checkpoint_position = should_save and CheckpointManager.has_active_checkpoint()

	if data.has_checkpoint_position:
		checkpoint = CheckpointManager.get_active_checkpoint()
...
static func _place_player_at_default(player: PlayerEntity) -> void:
	if not is_instance_valid(player):
		return

	player.global_position = CheckpointManager.get_default_spawn_position()
	player.velocity = Vector3.ZERO
	player.movement_controller.movement_enabled = true
	player.movement_controller.disable_timer = 0.0
