...
# TODO Confirm if this should be on _input instead of _unhandled_input
func _input(event: InputEvent) -> void:
	...
	if player_is_using_mouse:
		...
		_pitch = clamp(_pitch + deg_to_rad(-event.relative.y * vertical_sensibility), deg_to_rad(-85), deg_to_rad(45))
		rotation.x = _pitch
...
func _process(delta: float) -> void:
	...
	if look_input.length() > 0.1:  # small deadzone
		...
		# Limit vertical camera rotation
		_pitch = clamp(_pitch + deg_to_rad(v), deg_to_rad(-85), deg_to_rad(45))
		rotation.x = _pitch
#endregion
